  ...
  (let* ((name (function-call-name node))
    ...
    (cond
      ...
      (t
       ...
       (write-char #\) stream)
       (when (function-call-within-group node)
         (write-string " WITHIN GROUP (ORDER BY " stream)
...
(defgeneric emit-tablesample (dialect spec stream ctx)
  (:documentation "Emit TABLESAMPLE … after a table reference. Open for dialects.")
  (:method ((dialect sql-dialect) (spec tablesample-spec) stream ctx)
    (write-string " TABLESAMPLE " stream)
    (write-string (ecase (tablesample-method spec)
                    (:bernoulli "BERNOULLI")
                    (:system "SYSTEM"))
                  stream)
    (write-string " (" stream)
    (emit-sql dialect (lit (tablesample-percent spec)) stream ctx)
    (write-char #\) stream)
    (when (tablesample-repeatable spec)
      (write-string " REPEATABLE (" stream)
      (emit-sql dialect (lit (tablesample-repeatable spec)) stream ctx)
      (write-char #\) stream))))
...
(defun emit-window-spec (dialect w stream ctx)
...
(defmethod emit-sql (dialect (node over-expr) stream ctx)
  ...
  (let ((w (over-window node)))
    (cond
      ((typep w 'window-spec)
       (write-char #\( stream)
       (emit-window-spec dialect w stream ctx)
       (write-char #\) stream))
      (t
       (emit-ident dialect w stream)))))
...
(defgeneric emit-join (dialect join stream ctx)
  (:method (dialect join stream ctx)
    ...
    (when (join-tablesample join)
      (emit-tablesample dialect (join-tablesample join) stream ctx))
    (cond
      ((join-using join)
...
(defmethod emit-sql (dialect (stmt select-statement) stream ctx)
  ...
  (let ((f (%clause stmt 'from-clause)))
    (when f
      ...
      (when (from-tablesample f)
        (emit-tablesample dialect (from-tablesample f) stream ctx))))
  ...
  (let ((wins (%clauses stmt 'window-clause)))
    (when wins
      (write-string " WINDOW " stream)
      (loop for (wc . rest) on wins
            do (emit-ident dialect (window-clause-name wc) stream)
               (write-string " AS (" stream)
               (emit-window-spec dialect (window-clause-spec wc) stream ctx)
               (write-char #\) stream)
               (when rest (write-string ", " stream)))))
...
(defgeneric emit-column-def (dialect col stream ctx)
  (:method ((dialect sql-dialect) col stream ctx)
    ...
    (when (column-def-generated col)
      (write-string " GENERATED " stream)
      (write-string (ecase (column-def-generated col)
                      (:always "ALWAYS")
                      (:by-default "BY DEFAULT"))
                    stream)
      (write-string " AS (" stream)
      (emit-sql dialect (column-def-generated-as col) stream ctx)
      (write-char #\) stream)
      (when (column-def-stored col)
        (write-string " STORED" stream)))))

(defun %like-option-sql (opt)
  (ecase opt
    (:defaults "DEFAULTS")
...
(defgeneric emit-table-like (dialect like stream ctx)
  (:documentation "Emit LIKE other_table [INCLUDING|EXCLUDING …]. Open for dialects.")
  (:method ((dialect sql-dialect) (like table-like-clause) stream ctx)
    (declare (ignore ctx))
    (write-string "LIKE " stream)
    (emit-ident dialect (table-like-source like) stream)
    (dolist (opt (table-like-including like))
      (write-string " INCLUDING " stream)
      (write-string (%like-option-sql opt) stream))
    (dolist (opt (table-like-excluding like))
      (write-string " EXCLUDING " stream)
      (write-string (%like-option-sql opt) stream))))
    ...
    (when (or parts (null (create-table-of-type stmt)))
      ...
      (loop for (p . rest) on parts
            do (cond
                 ...
                 ((typep p 'table-like-clause)
                  (emit-table-like dialect p stream ctx))
                 (t
...
(defmethod emit-sql (dialect (stmt create-assertion-statement) stream ctx)
  ...
  (emit-ident dialect (create-assertion-name stmt) stream)
  (write-string " CHECK (" stream)
  (emit-sql dialect (create-assertion-check stmt) stream ctx)
  (write-char #\) stream)
  (when (create-assertion-deferrable stmt)
    (write-string " DEFERRABLE" stream))
  (when (create-assertion-initially stmt)
    (write-string " INITIALLY " stream)
    (write-string (ecase (create-assertion-initially stmt)
                    (:immediate "IMMEDIATE")
                    (:deferred "DEFERRED"))
                  stream)))

(defmethod emit-sql (dialect (stmt drop-assertion-statement) stream ctx)
  (declare (ignore ctx))
  (write-string "DROP ASSERTION " stream)
  (when (drop-assertion-if-exists stmt) (write-string "IF EXISTS " stream))
  (emit-ident dialect (drop-assertion-name stmt) stream)
  (when (drop-assertion-cascade stmt) (write-string " CASCADE" stream)))

(defgeneric emit-lock-table (dialect stmt stream ctx)
  (:documentation "Emit LOCK TABLE. Open — dialects may remap lock modes.")
  (:method ((dialect sql-dialect) (stmt lock-table-statement) stream ctx)
    (declare (ignore ctx))
    (write-string "LOCK TABLE " stream)
    (loop for (tbl . rest) on (lock-table-tables stmt)
          do (emit-ident dialect tbl stream)
             (when rest (write-string ", " stream)))
    (write-string " IN " stream)
    (write-string (ecase (lock-table-mode stmt)
    ...
    (when (lock-table-nowait stmt) (write-string " NOWAIT" stream))
    (when (lock-table-wait stmt)
      (write-string " WAIT " stream)
      (princ (lock-table-wait stmt) stream))))
...
(defmethod emit-sql (dialect (stmt create-collation-statement) stream ctx)
  (declare (ignore ctx))
  (write-string "CREATE COLLATION " stream)
  (when (create-collation-if-not-exists stmt) (write-string "IF NOT EXISTS " stream))
  (emit-ident dialect (create-collation-name stmt) stream)
  (cond
    ((create-collation-from stmt)
     (write-string " FROM " stream)
     (emit-ident dialect (create-collation-from stmt) stream))
    (t
     (write-string " (" stream)
     (let ((parts nil))
       (when (create-collation-locale stmt)
         (push (format nil "LOCALE = '~a'" (create-collation-locale stmt)) parts))
       (when (create-collation-lc-collate stmt)
         (push (format nil "LC_COLLATE = '~a'" (create-collation-lc-collate stmt)) parts))
       (when (create-collation-lc-ctype stmt)
         (push (format nil "LC_CTYPE = '~a'" (create-collation-lc-ctype stmt)) parts))
       (when (create-collation-provider stmt)
         (push (format nil "PROVIDER = ~a"
                       (ident-string (create-collation-provider stmt)))
               parts))
       (when (create-collation-deterministic stmt)
         (push (format nil "DETERMINISTIC = ~a"
                       (if (eq (create-collation-deterministic stmt) :false)
                           "FALSE" "TRUE"))
               parts))
       (write-string (format nil "~{~a~^, ~}" (nreverse parts)) stream))
     (write-char #\) stream)))
  (when (create-collation-pad-space stmt)
    (write-char #\Space stream)
    (write-string (ecase (create-collation-pad-space stmt)
                    ((t) "PAD SPACE")
                    (:no "NO PAD"))
                  stream)))

(defmethod emit-sql (dialect (stmt drop-collation-statement) stream ctx)
  (declare (ignore ctx))
  (write-string "DROP COLLATION " stream)
  (when (drop-collation-if-exists stmt) (write-string "IF EXISTS " stream))
  (emit-ident dialect (drop-collation-name stmt) stream)
  (when (drop-collation-cascade stmt) (write-string " CASCADE" stream)))

(defmethod emit-sql (dialect (stmt create-character-set-statement) stream ctx)
  (declare (ignore ctx))
  (write-string "CREATE CHARACTER SET " stream)
  (when (create-character-set-if-not-exists stmt) (write-string "IF NOT EXISTS " stream))
  (emit-ident dialect (create-character-set-name stmt) stream)
  (when (create-character-set-from stmt)
    (write-string " GET " stream)
    (emit-ident dialect (create-character-set-from stmt) stream))
  (when (create-character-set-collate stmt)
    (write-string " COLLATE " stream)
    (emit-ident dialect (create-character-set-collate stmt) stream)))

(defmethod emit-sql (dialect (stmt drop-character-set-statement) stream ctx)
  (declare (ignore ctx))
  (write-string "DROP CHARACTER SET " stream)
  (when (drop-character-set-if-exists stmt) (write-string "IF EXISTS " stream))
  (emit-ident dialect (drop-character-set-name stmt) stream)
  (when (drop-character-set-cascade stmt) (write-string " CASCADE" stream)))
